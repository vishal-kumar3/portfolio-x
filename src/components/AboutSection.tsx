...
const AboutSection = () => {
  ...
  useEffect(() => {
    if (!data || Object.keys(data).length === 0)
      return setActivityImage("/question.jpg");
    setUsername(data.discord_user.username);
    setDiscordOnline(data.discord_status);
    setActivityImage(
      `https://cdn.discordapp.com/avatars/${process.env.NEXT_PUBLIC_DISCORD_ID}/${data?.discord_user.avatar}.png?size=512`,
    );
  }, [data]);
  ...
  return (
    <section id="about" className="text-foreground p-4 w-full mx-auto">
      <h2 className="flex items-center justify-center gap-2 text-4xl font-semibold mx-auto w-fit mb-4 text-foreground">
        <Image src={"/icons/about.svg"} alt=" " width={32} height={32} />
        <span>code:about</span>
      </h2>
      ...
      <div className="md:flex md:space-x-4 md:items-center">
        ...
        <div className="mb-4 text-lg md:mb-0 md:w-1/2">
          <div className="flex justify-center items-center gap-6 sm:gap-10 md:gap-8">
            <Image
              ...
              height={128}
              alt="Kya pta"
              className="size-[8rem] rounded-[18px] bg-[#422006]"
            />
            <div className="leading-6 w-fit sm:w-52">
              ...
              <div className="sm:flex sm:flex-row sm:gap-2 text-md text-foreground">
                <p>{currTime.split(",")[0]}</p>
                <p>{currTime.split(",")[1]}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="md:w-1/2">
          <p className="text-lg text-center md:text-start">
            ...
            are both technically sound and user-friendly. Improving myself by
            learning golang and devops.
          </p>
        </div>
      </div>
    </section>
