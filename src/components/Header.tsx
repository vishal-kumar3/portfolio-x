...
const navigationTabs = [
  {
    ...
    icon: <BiHome />,
    onclick: () => {
      buttonClickEvent(ButtonEventName.HomeNavbar);
    },
  },
  {
    href: "/#about",
    ...
    icon: <IoInformation />,
    onclick: () => {
      buttonClickEvent(ButtonEventName.AboutNavbar);
    },
  },
  {
    href: "/#experience",
    ...
    icon: <PiFlaskFill />,
    onclick: () => {
      buttonClickEvent(ButtonEventName.SkillButton);
    },
  },
  {
    href: "/#achievement",
    topTitle: "achievement",
    title: "Achievement",
    icon: <Clock className="size-4" />,
    onclick: () => {
      buttonClickEvent(ButtonEventName.TimelineButton);
    },
  },
  {
    href: "/#work",
    ...
    icon: <CgWorkAlt />,
    onclick: () => {
      buttonClickEvent(ButtonEventName.WorkNavbar);
    },
  },
];
  ...
  return (
    <>
      <header className="flex items-center justify-center transition-all duration-300">
        ...
        <nav className="md:hidden rounded-t-xl text-md fixed z-50 bottom-0 left-0 right-0 bg-card backdrop-blur-sm">
          <div className="flex justify-around py-3">
            {navigationTabs.map((tab) => (
              <Link
                ...
                href={tab.href}
                onClick={tab.onclick}
                className="text-card-foreground rounded-md flex flex-col items-center hover:bg-card-hover hover:text-card-hover-foreground transition-colors px-2 py-2"
              >
                <div>{tab.icon}</div>
                <div>{tab.title}</div>
              </Link>
