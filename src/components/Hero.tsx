...
const ColorThemeToggle = dynamic(
  () => import("@/components/Theme/ColorThemeToggle"),
  {
    ssr: false,
  },
);
...
const HeroSection = () => {
  ...
  return (
    <section
      id="#home"
      className="relative flex flex-col md:gap-8 md:flex-row justify-between items-center py-10 md:py-20 overflow-hidden md:px-8"
    >
      {/* Left part */}
      <div className="relative flex flex-col items-center md:items-start z-10  pb-10 md:pb-0">
        <Tooltip tip="Click on Vishal to find Easter egg🥚" direction="bottom">
          <h1 className="group text-7xl z-5 md:text-8xl tracking-tighter font-bold mb-4 text-primary">
            <ColorThemeToggle>Visha</ColorThemeToggle>
            <span className="relative z-10 inline-flex items-center justify-center">
              ...
              <ThemeToggle
                size={20}
                className="absolute z-50 animate-[bounce_1s_ease-in-out_infinite] -top-3 mx-auto"
              />
            </span>
          </h1>
        </Tooltip>
            ...
            <Tooltip key={link.id} tip={link.tip}>
              ...
              >
                {link.icon({
                  size: 24,
                  className:
                    "w-[40px] h-[40px] p-2 rounded-md hover:bg-card-hover hover:text-card-hover-foreground",
                })}
              </Link>
            </Tooltip>
          ))}
        ...
        <div className="flex flex-col lg:flex-row gap-2">
          <Link
            href="/#work"
            onClick={() =>
              buttonClickEvent(ButtonEventName.DiscoverMyWorkButton)
            }
          ...
          <Link
            ...
            className="bg-card text-card-foreground flex items-center justify-center text-xl sm:hidden px-6 py-3 rounded-xl border border-card-foreground group-hover:bg-opacity-80 transition-colors relative overflow-hidden group"
          >
